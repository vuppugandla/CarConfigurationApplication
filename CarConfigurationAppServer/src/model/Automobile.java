...
public class Automobile implements Serializable {
	...
	public synchronized void deleteOptionSet(String name){	
		...
		optionImpl.deleteAllOptions(model, name);
		setImpl.deleteOptionSet(model, name);
	}
	...
	public synchronized void deleteOption(String optSetName, String optName){
		...
		//deleting from database
		OptionsImpl optionImpl = new OptionsImpl();
		optionImpl.deleteOption(model, optSetName, optName);
	}
	...
	public synchronized void updateOptionSet(String oldOptSetName, String newOptSetName){
		try{
			...
			OptionSetImpl setImpl = new OptionSetImpl();
			setImpl.updateOptionSet(model, oldOptSetName, newOptSetName);
	...
	public synchronized void updateOption(String optSetName, String oldOptName, String newOptName, float newPrice){
		try{
			Option ops = findOption(optSetName,oldOptName);
			ops.setOptionName(newOptName);
			ops.setOptionPrice(newPrice);
			//updating in database
			OptionsImpl optionImpl = new OptionsImpl();
			optionImpl.updateOption(model, optSetName, oldOptName, newOptName, newPrice);
		}catch(NullPointerException e){
			System.out.println("\nCannot find the model, optionset, option combination\n");
		}
	}
